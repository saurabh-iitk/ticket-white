...
@section('content')
    ...
    <main class="app-content">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-bar-chart"></i> Feedback Report</h1>
            </div>
        </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="est_id" id="event_show_time_id" style="width:100%;"
                                        onchange="get_layout_by_show_time(this.value)">
                                ...
                                @if ($bookings && $count == 2)
                                    @foreach ($bookings as $key => $booking)
                                        <tr>
                                            ...
                                            <td>{{$booking->is_feedback_sent}}</td>
                                            <td>{{$booking->feedback_value}}</td>
                                            <td>{{$booking->feedback_comment}}</td>
...
$totalFeedback = 0;
foreach ($bookings_response as $item) {
...
var ldata = [
<?php
foreach ($bookings_response as $value) {
    $color = $color_arr[$value->feedback_value];
    $percentage = $totalFeedback > 0 ? round(($value->count * 100) / $totalFeedback) : 0;
?>
    {
        value: {{$value->count}},
        color: "{{$color}}",
        highlight: "{{$color}}",
        label: "{{$value->feedback_value}}",
...
ldata.forEach(function(data) {
    legend1.innerHTML +=
        '<div><span style="width:26px;height:10px;position:absolute;margin:5px -35px;background-color:' +
        data.color +
        '"></span>' +
        data.label +
...
<?php
// ---------------- Feedback Sent Summary ----------------
$totalSummary = 0;
foreach ($bookings_summary as $item) {
    $totalSummary += $item->count;
}
...
<?php
$u = 0;
foreach ($bookings_summary as $value) {

    if ($value->is_feedback_sent == 'YES') {
        $value->is_feedback_sent = 'SENT';
    }
    if ($value->is_feedback_sent == 'NO') {
        $value->is_feedback_sent = 'NOT SENT';
    }

    $percentage = $totalSummary > 0 ? round(($value->count * 100) / $totalSummary) : 0;
?>
    {
        value: {{$value->count}},
        color: "{{$color[$u]}}",
        highlight: "{{$color[$u]}}",
        label: "{{$value->is_feedback_sent}}",
        percentage: "{{$percentage}}"
    },
<?php
    $u++;
...
var pieChart = new Chart(ctxp).Doughnut(pdata);
