...
@section('title', 'Event Summary Show Wise Report')
@section('css')
@endsection
...
<main class="app-content">
    <div class="app-title">
        <div>
            <h1><i class="fa fa-bar-chart"></i> Event Summary Show Wise Report</h1>
        </div>
    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                ?>
                                <select class="form-control" name="event_day[]" id="event_day" style="width:100%;" multiple>
                                    <option value="">All Day</option>
                                    <?php foreach($days as $day){?>
                                        <option value="<?php echo $day;?>" <?php 
                if (!empty($event_day) && is_array($event_day) && in_array($day, $event_day)) {
                    echo 'selected';
                } elseif ($event_day == $day) { // For single day (fallback)
                    echo 'selected';
                }  ?> ><?php echo $day;?></option>
                                    <?php }?>
                                </select>
                                <p>Note: Hold Ctrl for Multiple Selection</p>
                            </div>
                        </div>
                    ...
                    <table class="table table-hover table-bordered table-striped" id="show_date_table1" style="overflow:scroll; width: 100%;">
                        <thead>
                            <tr  style="background-color:#4ec5ed;">
                                <th class="text-center" colspan="15">Sale Summary</th>
                            </tr> 
                            <tr style="background-color:#c9eaf5">
                                ...
                                <th class="text-center">Received After Discount </th>
                                <th class="text-center">Total Discount Given</th>   
                        ...
                        <tbody>
                                    ...
                                    <tr>
                                        ...
                                        $upi_arr=array();
                                        $barter_arr=array();
                                        $discount_cash_upi=array();
                                        $complementary_arr=array();
                                        ...
                                        foreach($payment_method_data as  $single_method)
                                        {
                                            ...
                                            foreach ($single_data as  $single_summary)
                                            {
                                                ...
                                                if($single_summary['id']==$single_method['id'])
                                                {
                                                    ...
                                                    {
                                                        $complementary_arr[]=$paid_total;
                                                    }


                                                    // if($single_method['method_type']=='BARTER' )
                                                    // {
                                                    //     $barter_arr[]=$paid_total;
                                                    // }

                                                    if($single_method['method_type']=='DISCOUNT' )
                                                    {
                                                        $discount_cash_upi[]=$paid_total;
                                                    }
                                        ...
                                        $discount_cash_upi=array_sum($discount_cash_upi);
                                        $uc_total[]=$discount_cash_upi;
                                        // $discount_cash_upi=number_format($discount_cash_upi);

                                        // $net_amount_arr=array_sum($net_amount_arr);
                                        $net_amount_arr=$total_amount_arr-$discount_arr;
                                        ...
                                        $complementary_arr=array_sum($complementary_arr);
                                        $comp_total[]=$complementary_arr;
                                        // $upi_arr=number_format($upi_arr);


                                        $to_sale[]=$net_amount_arr-$complementary_arr;

                                    echo '<th class="text-center">'.number_format($cash_arr).'</th>';
                                    echo '<th class="text-center">'.number_format($upi_arr).'</th>';
                                    echo '<th class="text-center">'.number_format($discount_cash_upi).'</th>';
                                    echo '<th class="text-center">'.number_format($discount_arr).'</th>';
                                    echo '<th class="text-center">'.number_format($total_amount_arr).'</th>';
                                    echo '<th class="text-center">'.number_format($net_amount_arr-$complementary_arr).'</th>';
                                        ?>
                                    </tr>
                                    <?php } 
...
echo '<th class="text-center">'.number_format(array_sum($uc_total)).'</th>';
echo '<th class="text-center">'.number_format(array_sum($t_discount)).'</th>';
echo '<th class="text-center">'.number_format(array_sum($ts_sale)).'</th>';
echo '<th class="text-center">'.number_format(array_sum($to_sale)).'</th>';
// echo '<th class="text-center">'.number_format(array_sum($t_sale) - array_sum($uc_total)).'</th>';
                            echo '</tr>';
                            ?>
                    ...
                    <strong>Note: Net Total = Total Sale - Complementary - Discounts Given</strong>
